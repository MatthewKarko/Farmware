...
class UserManagerTestCases(TestCase):
    ...
    def test_create_user(self):
        ...
        Organisation.objects.create(
            code=org_code, name="nameoforg", logo="logoofog")
        organisation = Organisation.objects.get(name="nameoforg")
        user = get_user_model().objects.create_user(
            "email@gmail.com", "first_name", "last_name", org_code, None)
    ...
    def test_create_user_is_staff(self):
        ...
        user = get_user_model().objects.create_user("email@gmail.com", "first_name",
                                                    "last_name", org_code, None, is_staff=True)
        self.assertEqual(user.email, "email@gmail.com")
        self.assertTrue(user.is_active)
    ...
    def test_create_superuser(self):
        ...
        user = get_user_model().objects.create_superuser(
            "email@gmail.com", "first_name", "last_name", org_code)
        self.assertEqual(user.email, "email@gmail.com")
...
class UserSerialiserTests(TestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            "email@gmail.com", "first_name", "last_name", Org)
        self.serializer = UserSerialiser(self.user)

    def test_contains_expected_fields(self):
        data = self.serializer.data
        self.assertEqual(set(data.keys()), set(
            ['id', 'email', 'first_name', 'last_name', 'organisation', 'role', 'teams']))
...
class UserUpdateSerialiserTests():
    def setUp(self):
        ...
        response['role'] = 400
        self.user = get_user_model().objects.create_user(email="email@gmail.com", first_name="first_name",
                                                         last_name="last_name", organisation=Org, password="password123", role=400)
...
class LoginSerialiserTests():
    def seUp(self):
        ...
        Org = Organisation.objects.get(name="nameoforg")
        self.user = get_user_model().objects.create_user(
            "email@gmail.com", "first_name", "last_name", Org, "password123")
        self.serializer = UserSerialiser(self.user)
...
class RegisterUserSerialiserTests():
    def setUp(self):
        ...
        Organisation.objects.create(
            code=orgcode, name="nameoforg", logo="logoofog")
        Org = Organisation.objects.get(name="nameoforg")
        self.user = get_user_model().objects.create_user(
            "email@gmail.com", "first_name", "last_name", Org, "password123")
        serializer = UserSerialiser(user)
    ...
    def test_testing_create(self):
        ...
        rus = RegisterUserSerialiser(rs)
        Org = Organisation.objects.get(name="nameoforg")
        serializer_data = {'email': 'example@gmail.com', 'first_name': 'firstn',
                           'last_name': 'lastn', 'password': 'passwd', 'organisation': Org}
...
class UserViewSetTestCases(APITestCase):
    ...
    def test_testRegisteringUser(self):
        org = Organisation.objects.get(name="nameoforg")
        response = self.client.post('/api/user/ /register/user/', {
                                    'email': 'example@gmail.com', 'first_name': 'firstn', 'last_name': 'lastn', 'password': 'passwd', 'org_code': org.code})
    ...
    def test_RegisteringUserError(self):
        org = Organisation.objects.get(name="nameoforg")
        response = self.client.post('/api/user/ /register/user/', {
                                    'first_name': 'firstn', 'last_name': 'lastn', 'password': 'passwd', 'organisation': org})
    ...
    def test_RegisteringAdmin(self):
        ...
        org = Organisation.objects.get(name="nameoforg")
        response = self.client.post('/api/user/ /register/admin/', {
                                    'email': 'example@gmail.com', 'first_name': 'firstn', 'last_name': 'lastn', 'password': 'passwd', 'org_name': org.pk})
        self.assertEquals(response.status_code, 201)

    def test_RegisteringAdminError(self):
        serializer_data = {}
        response = self.client.post(
            '/api/user/ /register/admin/', serializer_data)
    ...
    def test_estingSettingPasswordsError(self):
        ...
        serializer_data = {'email': 'example@gmail.com', 'first_name': 'firstn',
                           'last_name': 'lastn', 'password': 'passwd', 'organisation': org}
        response = self.client.post(
            '/api/user/ /set_password/', serializer_data)
        self.assertEquals(response.status_code, 405)

    def test_testingSettingPasswords(self):
        org = Organisation.objects.get(name="nameoforg")
        serializer_data = {'email': 'example@gmail.com', 'first_name': 'firstn',
                           'last_name': 'lastn', 'password': 'passwd', 'org_code': org.code}
        response = self.client.post(
            '/api/user/ /register/user/', serializer_data)
        self.assertEquals(response.status_code, 201)
        user = User.objects.first()
        self.client.force_authenticate(user)
        serializer_data = {'email': 'example@gmail.com', 'first_name': 'firstn', 'last_name': 'lastn',
                           'old_password': 'passwd', 'new_password': 'pass', 'org_code': org.code}
        respons = self.client.get(
            '/api/user/ /set_password/', serializer_data, args={'pk': user.pk})
    ...
    def test_testingDestroy(self):
        ...
        response = self.client.post('/api/user/ /register/user/', {
                                    'email': 'example@gmail.com', 'first_name': 'firstn', 'last_name': 'lastn', 'password': 'passwd', 'org_name': org.name})
        self.assertEquals(response.status_code, 201)
    ...
    def test_testingList(self):
        org = Organisation.objects.get(name="nameoforg")
        serializer_data = {'email': 'example@gmail.com', 'first_name': 'firstn',
                           'last_name': 'lastn', 'password': 'passwd', 'org_name': org.name, 'role': 000}
    ...
    def test_testingMyteams(self):
        ...
        team = Team.objects.create(
            category="sports", name="fifa team 12", organisation=org)
        response = self.client.post('/api/user/ /register/user/', {
                                    'email': 'example@gmail.com', 'first_name': 'firstn', 'last_name': 'lastn', 'password': 'passwd', 'org_code': org.code})
        self.assertEquals(response.status_code, 201)
    ...
    def test_testingUserteams(self):
        ...
        team = Team.objects.create(
            category="sports", name="fifa team 12", organisation=org)
        serializer_data = {'email': 'example@gmail.com', 'first_name': 'firstn',
                           'last_name': 'lastn', 'password': 'passwd', 'org_name': org.name, 'team': team}
        response = self.client.post(
            '/api/user/ /register/admin/', serializer_data)
        self.assertEquals(response.status_code, 201)
    ...
    def testingPartialUpdate(self):
        ...
        # print(response.content)
        self.assertEquals(response.status_code, 201)
        respons = self.client.patch(
            f'/api/user/ /{user.pk}/partial_update/', {'first_name': 'firstn'})
        # print(respons.content)
...
class ActivateAccountTestCases(APITestCase):
    ...
    def test_testingGet(self):
        ...
        respons = self.client.post(
            f'/api/user/ /activate/{uidb64}/{token}/', {'user': user})
        self.assertEquals(respons.status_code, 200)
